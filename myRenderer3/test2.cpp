...
Array Array::createNewArray(unsigned int sizeElem)
{
    Array a;
    a.m_Data = nullptr;
    a.m_Size = 0;
...
bool Array::isArrayEmpty(const Array* const arr)
{
    assert(arr);
    return 0 == arr->m_Size;
}

int Array::getArraySize(const Array* const arr)
{
    assert(arr);
    return arr->m_Size;
}

void Array::addArraySize(Array* const arr, int increment)
{
    assert(arr);
    assert(increment);
...
int Array::getArrayCapacity(const Array* const arr)
{
    assert(arr);
    return arr->m_Capacity;
}

int Array::getArrayTypeSize(const Array* const arr)
{
    assert(arr);
    return arr->m_TypeSize;
}

void Array::clearArray(Array* const arr)
{
    assert(arr);
    arr->m_Size = 0;
}
...
void Array::checkArraySuffMem(Array *const dstArr, void * ptr)
{
    assert(dstArr);
    assert(ptr);
...
void Array::pushBackArray(Array* const dstArr, const void* srcData)
{
    assert(dstArr);
    assert(srcData);
...
void Array::removeAtFastArray(Array* const arr, unsigned int index)
{
    assert(!isArrayEmpty(arr));
    assert(index >= 0 && index < arr->m_Size);
    unsigned char* ptr = (unsigned char*)arr->m_Data;
